      ...
      <table className="font-sans border-collapse text-[13px]">
        <thead>
          <tr>
            ...
            {CONFIGS.map(config => (
              <th key={config} className="pb-3 px-4 text-center text-[10px] uppercase tracking-widest text-stone-400 font-normal min-w-[120px]">
                {config}
              </th>
            ))}
          </tr>
              ...
              {CONFIGS.map(config => {
                ...
                return (
                  <td key={config} className="py-3 px-4 text-center">
                    <span className="inline-block border border-stone-200 dark:border-stone-800 bg-white/50 dark:bg-stone-900/50 px-3 py-1.5 text-[11px] text-stone-500 dark:text-stone-400 font-mono">
                      {id}
                    </span>
                  </td>
                )
