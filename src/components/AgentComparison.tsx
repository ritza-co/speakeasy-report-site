...
function StatCard({ label, value, suffix = '', color }: {
  ...
  return (
    <div className="text-center">
      ...
      <p className="text-[10px] uppercase tracking-widest text-stone-600 dark:text-stone-400 mt-1">{label}</p>
    </div>
  )
}
  ...
  return (
    <div className="w-full space-y-10">
      ...
      <div className="border border-stone-200 dark:border-stone-850 bg-white dark:bg-stone-900 p-6">
        <p className="text-[10px] uppercase tracking-widest text-stone-600 dark:text-stone-400 mb-6">
          ...
          <table className="w-full text-sm font-sans border-collapse">
            <thead>
              <tr className="border-b border-stone-200 dark:border-stone-850">
                <th className="text-left py-2 pr-6 text-[10px] uppercase tracking-widest text-stone-600 dark:text-stone-400 font-normal">Method</th>
                {MODELS.map(m => (
            ...
            <tbody>
                  ...
                  {MODELS.map(model => {
                    ...
                    return (
                      <td key={model} className="py-3 text-center text-sm">
                        ...
                        <span className="text-stone-600 dark:text-stone-400 text-[11px] ml-1">({avg} avg)</span>
                      </td>
                    )
                  })}
                </tr>
              ...
              <tr className="border-t border-stone-200 dark:border-stone-850">
                <td className="py-3 pr-6 text-[11px] uppercase tracking-widest text-stone-600 dark:text-stone-400">Total</td>
                {MODELS.map(model => {
          ...
          <table className="w-full text-sm font-sans border-collapse">
            ...
            <tbody>
              {METHODS.map((method, i) => (
                ...
                >
                  <td className="py-3 text-[11px] uppercase tracking-widest text-stone-600 dark:text-stone-400">{method}</td>
                  {MODELS.map(model => {
                    const agg = aggregate(BENCHMARKS.filter(r => r.model === model && r.method === method))
                    if (!agg) return <td key={model} className="py-3 text-center text-stone-600 dark:text-stone-400">—</td>
        ...
        {MODELS.map((model: Model) => {
          ...
          return (
            <div key={model}
              ...
              <p className="text-[10px] uppercase tracking-widest text-stone-600 dark:text-stone-400 mb-4">{model}</p>
              <div className="space-y-3">
                ...
                <div className="text-center">
                  ...
                  <p className="text-[10px] uppercase tracking-widest text-stone-600 dark:text-stone-400 mt-1">avg time</p>
                </div>
